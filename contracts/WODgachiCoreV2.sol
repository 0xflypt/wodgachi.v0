...
/**
 * @title WODgachiCoreV2
 * @dev Enhanced core contract with token upgrade capability
 */
contract WODgachiCoreV2 {
    ...
    address public newCrushToken;
    bool public tokenUpgradeEnabled = false;
    ...
    event TokenUpgradeEnabled(address indexed newToken);
    ...
    function submitWorkout(
        ...
        // Reward tokens (works with both mock and real CRUSH)
    ...
    // Token upgrade functionality
    function enableTokenUpgrade(address _newCrushToken) external onlyOwner {
        require(_newCrushToken != address(0), "Invalid token address");
        newCrushToken = _newCrushToken;
        tokenUpgradeEnabled = true;
        emit TokenUpgradeEnabled(_newCrushToken);
    }
    
    function upgradeToNewToken() external onlyOwner {
        require(tokenUpgradeEnabled, "Token upgrade not enabled");
        require(newCrushToken != address(0), "New token not set");
        crushToken = IWODgachiToken(newCrushToken);
    }
